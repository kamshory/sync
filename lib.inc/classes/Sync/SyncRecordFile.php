<?php
namespace Sync;
class SyncRecordFile extends \Sync\SyncRecord
{
    ...
    public function createQuery($page)
    {
        ...
        $limit = $this->getLimit();
        return "SELECT * FROM `edu_sync_file` ORDER BY `time_create` DESC LIMIT $offset, $limit";
    }

    public function createQueryCount()
    {
        return "SELECT `sync_file_id` FROM `edu_sync_file` ";
    }
